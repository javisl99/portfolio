...
export function ApproachGrid({ items, locale }: { items: CopyCard[]; locale: Locale }) {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
      <article className="rounded-2xl border border-white/10 bg-[#102022] p-8 text-white shadow-[0_28px_60px_-32px_rgba(2,6,23,0.72)]">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
        {supportingItems.map((item, index) => {
          ...
          return (
            <article
              className={cn(
                "rounded-2xl border border-line bg-panel p-6 shadow-soft transition duration-200 hover:border-accent/35",
