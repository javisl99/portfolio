        ...
        <div className="space-y-8">
          {experienceEntries.map((entry, index) => (
            ...
            >
              ...
              <div
                className={cn(
                  "ml-12 w-full rounded-2xl border border-line bg-panel p-6 shadow-soft md:ml-0 md:w-[calc(50%-2.5rem)]",
                  index % 2 === 0 ? "md:mr-[2.5rem]" : "md:ml-[2.5rem]",
                )}
              >
                <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                  <p className="text-xs font-bold uppercase tracking-[0.22em] text-accent">{entry.progression[locale]}</p>
                ...
                <p className="mt-3 text-sm leading-7 text-muted">{entry.signal[locale]}</p>
              </div>
            </article>
          ))}
        </div>
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-3">
      {experienceEntries.map((entry) => (
        <article
          className="rounded-2xl border border-line bg-panel p-7 shadow-soft transition duration-300 hover:-translate-y-1 hover:border-accent/35"
        ...
        >
          ...
          <p className="mt-4 rounded-xl border-l-4 border-accent bg-surface/80 px-4 py-3 text-sm leading-7 text-ink">{entry.signal[locale]}</p>
          <div className="mt-5 flex flex-wrap gap-2">
            {entry.stack.slice(0, 3).map((item) => (
              <span className="rounded-md bg-surface px-3 py-1 text-xs font-medium text-muted" key={item}>
                {item}
              </span>
            ))}
          </div>
