  ...
  return (
    <div className="md:hidden">
      ...
      {mounted ? (
        <div
          aria-hidden={!open}
          className={`fixed inset-x-0 top-[4.75rem] z-40 border-b border-line bg-[rgba(3,7,18,0.94)] px-5 pb-6 pt-4 shadow-[0_28px_60px_-36px_rgba(2,6,23,0.98)] backdrop-blur-2xl transition-all duration-240 motion-reduce:transition-none ${
          ...
          <div className="mx-auto flex max-w-7xl flex-col gap-5">
            ...
            <nav aria-label="Mobile" className="grid gap-2">
              {copy.navigation.map((item) => (
                <Link
                  className="cursor-pointer rounded-2xl border border-line bg-panel px-4 py-3.5 text-sm font-semibold text-ink transition hover:border-accent/40 hover:bg-white/[0.04] hover:text-white focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-accent"
