  ...
  return (
    ...
    >
      <Container className="relative">
        <div className={cn("grid gap-6 sm:gap-8", hasBelowPanel ? "max-w-5xl" : "lg:grid-cols-[minmax(0,1.95fr)_20rem] lg:items-start lg:gap-10")}>
          <div className="max-w-4xl space-y-3 sm:space-y-5">
            ...
            <h1 className={cn("font-display text-[2.6rem] font-black leading-[0.98] tracking-tight text-ink sm:text-5xl lg:text-6xl", titleClassName)}>{title}</h1>
            <p className={cn("max-w-3xl text-base leading-7 text-muted sm:text-xl sm:leading-8", introClassName)}>{intro}</p>
          </div>
          {children ? (
            <div
              className={cn(
                "rounded-[1.8rem] border border-white/10 bg-[linear-gradient(180deg,rgba(13,21,39,0.95),rgba(8,13,26,0.98))] p-5 shadow-[0_30px_70px_-44px_rgba(15,23,42,0.95)] sm:p-6",
